 ...
 */
public class UpdateOrderDateController {
	...
	@FXML private Label lblPlacingDate;
	...
	@FXML private TextField txtPlacingDate;
	...
	public void UpdateBtn(ActionEvent event) {
	    ...
	    String newInfo = txtOrderNumber.getText() + " " + txtOrderDate.getText();
	    updateHashMap.put("UpdateOrderDate", newInfo);
	...
	public void Loadbtn(ActionEvent event) {
		...
		if (idnumber.matches("\\d+")) {
			...
			if (!ChatClient.fromserverString.contains(",")) {
				lblErr.setText("Cant load the requested OrderNumber. Please make sure you entered the right OrderNumber.");
			} else {
				lblErr.setText("Requested OrderNumber loaded.\nYou can update ONLY the order date:");
				loaded = 1;
			...
			lblErr.setText("Please enter OrderNumber that contains ONLY digits");
		}
	}
